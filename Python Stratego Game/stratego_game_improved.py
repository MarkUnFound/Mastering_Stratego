"""
Stratego with MARQ - Enhanced Edition
Features:
- How to Play manual (press 'M' key)
- Improved contrast: darker piece backgrounds for better visibility
- Piece inventory counter showing remaining pieces by type
- Larger rank numbers on pieces
- Battle indicators in move history showing winners
"""
...
lost_pieces_player1 = []
lost_pieces_player2 = []
show_manual = False  # New: manual overlay toggle
show_piece_numbers = False  # Toggle: show rank numbers instead of images (press A)
...
fullscreen = False
pygame.display.set_caption("Stratego - with MARQ (F11 for Fullscreen, M for Manual)")
...
# Player 1 - Blue Team (DARKER for better contrast on light tiles)
PLAYER1_PRIMARY = (30, 64, 175)      # Darker blue
PLAYER1_LIGHT = (59, 130, 246)
PLAYER1_DARK = (20, 40, 120)
PLAYER1_GLOW = (147, 197, 253)

# Player 2 - Red Team (DARKER for better contrast)
PLAYER2_PRIMARY = (180, 30, 30)      # Darker red
PLAYER2_LIGHT = (239, 68, 68)
PLAYER2_DARK = (120, 20, 20)
...
# Board colors - LIGHTER tiles for better piece contrast
TILE_LIGHT = (245, 245, 235)   # Cream/beige
TILE_DARK = (210, 180, 140)    # Tan
...
def calculate_history_width(current_width, board_width_with_borders, game_state=None):
    ...
    if remaining_space > MIN_HISTORY_WIDTH:
        return min(remaining_space - 10, 250)  # Wider for battle indicators
    return 0
...
def draw_piece(piece, x, y, size, highlight=False, semi_transparent=False, revealed=False):
    """Draw piece with LARGER numbers for better visibility"""
    if not piece:
        return
    ...
    if semi_transparent and not revealed:
        # Hidden enemy piece — always show ?? regardless of show_piece_numbers
        draw_rounded_rect(screen, GREY_700, piece_rect, radius=6, alpha=200)
        ...
        # Visible piece — respect show_piece_numbers toggle
        if (piece_type, player) in piece_images and not show_piece_numbers:
            # Draw image
            img = pygame.transform.smoothscale(piece_images[(piece_type, player)],
                                              (piece_rect.width, piece_rect.height))
            screen.blit(img, piece_rect.topleft)
        else:
            # Draw piece with rank number using the correct owner's color
    ...
    for r in range(BOARD_SIZE):
        for f in range(BOARD_SIZE):
            ...
            if piece:
                ...
                if visible:
                    draw_piece(piece, x, y, tile_size, highlight=is_selected, revealed=piece.revealed)
                else:
...
def count_pieces_by_type(player):
    """Count remaining pieces for each type"""
    counts = {}
    for r in range(BOARD_SIZE):
...
def draw_piece_inventory():
    """NEW: Draw piece counter showing remaining pieces by type"""
    dims = get_board_dimensions()
    current_width, current_height = screen.get_size()
    
    # Position below lost pieces tracker
    inv_x = dims['board_start_x']
    inv_y = 60
    inv_width = dims['board_width_with_borders']
    inv_height = 80
    
    # Background panel
    inv_rect = pygame.Rect(inv_x, inv_y, inv_width, inv_height)
    draw_rounded_rect(screen, CARD_BG, inv_rect, radius=10)
    pygame.draw.rect(screen, GREY_700, inv_rect, 2, border_radius=10)
    
    # Title
    draw_text("Piece Inventory", inv_rect.centerx, inv_y + 15, get_font_size(14), WHITE, bold=True)
    
    # Player 1 pieces
    p1_counts = count_pieces_by_type(1)
    p1_y = inv_y + 35
    p1_text = " | ".join([f"{k}:{v}" for k, v in sorted(p1_counts.items())])
    draw_text(f"Blue: {p1_text}", inv_rect.centerx, p1_y, get_font_size(11), PLAYER1_LIGHT)
    
    # Player 2 pieces
    p2_counts = count_pieces_by_type(2)
    p2_y = inv_y + 55
    p2_text = " | ".join([f"{k}:{v}" for k, v in sorted(p2_counts.items())])
    draw_text(f"Red: {p2_text}", inv_rect.centerx, p2_y, get_font_size(11), PLAYER2_LIGHT)

def draw_move_history():
    """Draw move history with BATTLE INDICATORS"""
    dims = get_board_dimensions()
    ...
    move_height = get_font_size(14) + 6
    visible_moves = int((history_height - header_height - 20) / move_height)
    start_index = max(0, len(move_history) - visible_moves)
    for i, move_entry in enumerate(move_history[start_index:], start=start_index + 1):
        move_color = get_player_color(1 if i % 2 == 1 else 2, 'light')
        
        # Check if this is a battle move (has ' vs ' in it)
        if ' vs ' in move_entry:
            # Extract winner indicator if present
            if '→' in move_entry:
                parts = move_entry.split('→')
                move_text = parts[0].strip()
                winner = parts[1].strip()
                # Show move with winner icon
                draw_text(f"{i}. {move_text}", history_rect.centerx - 15, move_y,
                          get_font_size(10), move_color, center=True)
                # Winner indicator
                if 'Blue' in winner or '1' in winner:
                    indicator = "🔵"
                elif 'Red' in winner or '2' in winner:
                    indicator = "🔴"
                else:
                    indicator = "⚔"
                draw_text(indicator, history_rect.right - 20, move_y,
                          get_font_size(12), WHITE, center=True)
            else:
                draw_text(f"{i}. {move_entry}", history_rect.centerx, move_y,
                          get_font_size(10), move_color, center=True)
        else:
            draw_text(f"{i}. {move_entry}", history_rect.centerx, move_y,
                      get_font_size(10), move_color, center=True)
...
def draw_manual():
    """NEW: Draw the how-to-play manual overlay"""
    if not show_manual:
        return
    ...
    # Manual panel
    manual_w = min(700, current_width - 100)
    manual_h = min(600, current_height - 100)
    manual_x = (current_width - manual_w) // 2
    manual_y = (current_height - manual_h) // 2
    
    manual_rect = pygame.Rect(manual_x, manual_y, manual_w, manual_h)
    draw_rounded_rect(screen, CARD_BG, manual_rect, radius=15)
    pygame.draw.rect(screen, ACCENT, manual_rect, 3, border_radius=15)
    
    # Title
    draw_text("HOW TO PLAY STRATEGO", manual_rect.centerx, manual_y + 30,
              get_font_size(28), ACCENT, bold=True)
    ...
    content = [
        "OBJECTIVE:",
        "• Capture your opponent's Flag or eliminate all movable pieces",
        "",
        "SETUP:",
        "• Each player has 40 pieces: 1 Flag, 6 Bombs, and 33 other ranks",
        "• Arrange pieces in your territory (bottom 4 rows for Player 1)",
        "",
        "PIECE RANKS (Higher beats Lower):",
        "• Marshal (10) - Highest rank, but loses to Spy",
        "• General (9), Colonel (8), Major (7), Captain (6)",
        "• Lieutenant (5), Sergeant (4), Miner (3), Scout (2), Spy (1)",
        "• Special: Spy (1) defeats Marshal (10)",
        "• Special: Miner (3) defuses Bombs",
        "• Scout (2) can move multiple squares",
        "• Flag & Bombs cannot move",
        "",
        "MOVEMENT:",
        "• Move one square (up/down/left/right) per turn",
        "• Scouts can move multiple empty squares in a straight line",
        "• Cannot move onto lakes or your own pieces",
        "",
        "COMBAT:",
        "• Attack by moving onto opponent's square",
        "• Higher rank wins; equal ranks = both eliminated",
        "• Defeated pieces are removed from the board",
        "",
        "CONTROLS:",
        "• Click piece → Click destination to move",
        "• M: Toggle this manual",
        "• A: Toggle piece numbers / images",
        "• H: Toggle move history",
        "• F11: Toggle fullscreen",
    ]
    
    for line in content:
        if y_pos > manual_y + manual_h - 60:
            break
        size = get_font_size(13) if line.startswith("•") else get_font_size(14)
        bold = not line.startswith("•")
        color = WHITE if not line.startswith("•") else GREY_300
        draw_text(line, manual_x + 30, y_pos, size, color, bold=bold, center=False)
        y_pos += line_height
    
    # Close instruction
    draw_text("Press M or ESC to close", manual_rect.centerx, manual_y + manual_h - 25,
              get_font_size(14), GREY_500)
    ...
    if game_state == "setup":
        ...
        if selected is None:
            ...
            if sq in board.legal_moves_from(selected):
                ...
                # Snapshot for popup and history
                ...
                # Determine winner for history indicator
                if defender:
                    if piece_after_dst and piece_after_dst.owner == atk_owner:
                        result = 'attacker_wins'
                        winner_text = f"Blue wins" if atk_owner == 1 else "Red wins"
                    elif not piece_after_dst and not piece_after_src:
                        result = 'tie'
                        winner_text = "Tie"
                    else:
                        result = 'defender_wins'
                        winner_text = f"Blue wins" if def_owner == 1 else "Red wins"
                    
                    # Add winner indicator to move history
                    move_notation += f" → {winner_text}"
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_F11:
                ...
                show_manual = not show_manual
            elif event.key == pygame.K_a:
                show_piece_numbers = not show_piece_numbers
            elif event.key == pygame.K_ESCAPE:
                if game_state == "play" and not show_manual:
                    game_state = "pause"
            # TEST: Press 'V' to show victory screen
            elif event.key == pygame.K_v:
                game_state = "game_over"
                message = "You Win! "
        elif event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if not show_manual and game_state in ["setup", "play"]:
    ...
    if game_state == "menu":
        ...
        if draw_button("Quit", center_x - button_width//2, start_y + button_spacing * 3,
            ...
            running = False
        draw_text("Press F11 for fullscreen | Press M for How to Play", center_x, current_height - 30, get_font_size(14), GREY_500)
        ...
        draw_piece_inventory()  # NEW: Show piece counts
        draw_move_history()
        if vs_bot and current_player != human_side:
            bot_turn()
        ...
        if dims['history_width'] > 0:
            mode_label = "Numbers" if show_piece_numbers else "Images"
            hint_text = f"Press H to {'show' if not show_history_panel else 'hide'} history | A: {mode_label} | M for Manual"
    ...
    # Draw manual overlay last (on top of everything)
    draw_manual()
