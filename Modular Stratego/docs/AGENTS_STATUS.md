...
- **Data Mining Analysis (37,213 Episodes):** Formal analysis proved the Vanilla DQN+LSTM architecture has hit an architectural ceiling at 22% win rate. Six findings documented: (1) Win rate plateau since ep 2,000; (2) Loss-performance decoupling (loss→0.001, WR flat); (3) Q-value overestimation (0.2→1.2 with no performance gain); (4) LSTM accuracy ceiling at ~18-22%; (5) 15.2% WR vs greedy heuristic (worst opponent); (6) 38.5% draw dominance. Each finding was analytically traced to specific DQN architectural properties (maximization bias, scalar value collapse, sequential information bottleneck, exploration entropy death, sparse reward propagation). Updated `results.tex` and `recommendations.tex` with three-pronged argumentation (empirical + analytical + inductive). Rainbow DQN and AAREN components are marked as "Proposed" — empirical validation remains future work.
- **Bibliography Modernization (references.bib):** Audited the LaTeX references to remove 16 redundant/obsolete sources published in 2019 or earlier (e.g., original AdamW, AlphaStar foundational papers). Replaced them with modern equivalents (2020-2025) including recent MARL video game surveys, Hanabi agent (R3D2) papers, and verified AdamW constraint optimization research. This enforces a high standard of relevancy for the final paper's citations.
- **Python Shell Quoting (Windows PowerShell):** When executing inline Python scripts via `python -c` in PowerShell, single quotes often fail due to parser differences. To avoid `SyntaxError`, always write complex API fetching scripts (e.g., arXiv or Crossref scrapers) to a temporary `.py` file instead of passing them directly into the terminal argument string.
- **Manuscript Revision (Ch 5/6 — Results & Recommendations):** Reviewer feedback demanded removing chapter introductions and substantiating every title component with concrete evidence. Key editorial decisions: (1) Removed 4 preliminary sections from `results.tex` (Phase 1/2 summaries, piece inference overview, stalemate analysis) that read as preamble without quantitative substance. (2) Added "Evidence for Architectural Necessity" table mapping each MARQ title word (Multi-Agent, Rainbow DQN, AAREN, Imperfect Information) to specific training data findings. (3) Added "Behavioral Patterns from Training Logs" analyzing passive shuffling convergence, opponent adaptation failure, loss-reward divergence, and LSTM accuracy as information barrier. (4) Added "Architectural Comparison: Unified vs. Dual-Model" comparing the old SetupAgent+AttackDQN pipeline with the current unified DQN, explaining why both converge to draws for different structural reasons. (5) Added "Draw Dominance and Learning Signal Quality" framing losses as 3.3× stronger feedback than draws, explaining why the agent's draw-seeking behavior suppresses its own learning signal. (6) Removed LSTM-vs-DRQN introduction from `recommendations.tex` and added "Computer Science Contributions" section highlighting transferable CS contributions (five-phase curriculum, gradient bridge, Expectamax search, PBRS annealing). (7) Refocused Limitations to frame Stratego as test domain, with human testing contingent on time/resources. All text uses formal academic prose — no em-dashes, no bullet points, no bold formatting.
