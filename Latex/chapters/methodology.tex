...
\end{equation}
where weights are established as $w_o = 1.0$, $w_m = 0.5$, $w_e = 0.3$, and $w_p = 0.2$. This hierarchy ensures that the primary objective (winning) remains paramount while intermediate tactical goals provide a dense signal to bridge the credit assignment gap.
...
The choice of a normalized range $[V_{min}, V_{max}] = [-3, 3]$ is fundamentally linked to the use of C51 distributional RL. A narrow, well-defined support allows the 51 atoms to resolve fine-grained differences in value estimates ($\delta_z \approx 0.12$). If rewards were not normalized, large cumulative returns would exceed the support bounds, leading to "saturation" where the agent cannot distinguish between varying high-value outcomes. Specifically:
    \item \textbf{Terminal Outcomes}: Terminal rewards are normalized to $\pm 1.0$ for wins and losses. A draw receives $-1.5$, establishing it as a significantly worse outcome than a fighting loss to discourage stalling or piece-shuffling behaviors that prolong the game without strategic progress.
    \item \textbf{High-Level Time Pressure}: To incentivize efficient victories, a non-linear step penalty is applied based on the episode duration. The penalty begins at $-0.001$ per step, increasing to $-0.002$ after 500 steps, and reaching a critical pressure of $-0.005$ after 1000 steps. This "urgency" signal forces the agent to resolve games before the penalty accumulation outweighs the potential win reward.
...
\begin{itemize}
    \item \textbf{Time Pressure}: A constant step penalty of $-0.001$ creates a slight pressure toward efficiency.
    \item \textbf{Stalemate Prevention}: Sudden mobility restrictions (dropping below 50\% of available moves) trigger a $-0.05$ penalty, discouraging the agent from boxing itself into immovable positions.
