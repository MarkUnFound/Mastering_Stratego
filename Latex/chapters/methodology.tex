...
The framework aggregates weight reward categories using a linear combination.
\begin{equation}
R_{total} = w_o R_{outcome} + w_m R_{material} + w_e R_{epistemic} + w_p R_{positional} + R_{flag} + R_{curiosity}
\end{equation}
where weights are established as $w_o = 1.0$, $w_m = 1.0$, $w_e = 0.5$, and $w_p = 0.3$. The flag-distance reward ($R_{flag}$) and curiosity bonus ($R_{curiosity}$) are applied directly without component-level weighting. This hierarchy ensures that the primary objective (winning) remains paramount, while auxiliary tactical signals provide gradient-visible guidance within the C51 atom resolution.

\paragraph{Rationale for Distributional Scaling.}
The support range $[V_{min}, V_{max}] = [-10, 10]$ with 51 atoms yields a resolution of $\delta_z = 0.4$ per atom. Terminal outcomes are set to $\pm 1.0$ (flag capture win/loss), $+0.5$ (depletion win), and $-0.3$ (draw), which occupy 2--3 atoms each and produce meaningful distributional shifts. Per-step shaping rewards range from $0.01$ to $0.3$, spanning at least one atom and ensuring the network can distinguish captures, positional advances, and penalties from noise.

Piece-loss penalties are rank-weighted: losing a Marshal incurs $-0.1 \times (10/10) = -0.1$, while losing a Scout costs $-0.1 \times (2/10) = -0.02$, reflecting the asymmetric strategic impact of high-rank piece losses. Capture rewards are similarly scaled by defender rank. A constant step penalty of $-0.002$ per step provides time pressure; over a 500-step game, this accumulates to $-1.0$, comparable to the terminal reward and encouraging efficient play.

The forward-movement incentive ($R_{flag} = +0.02$) is gated on piece rank: only offensive pieces with rank $\leq 6$ (Captain and below) receive this bonus. High-value pieces (Marshal, General, Colonel) are not incentivized to advance, preventing suicidal rushes. Scout penetration bonuses apply only within the actual enemy back rank (2 rows), not the broader enemy half.

This reward system underwent multiple revisions. Early versions placed reward logic inside the game environment, causing confusion and reward spikes. Version 2 (current) rescaled all rewards for C51 atom visibility, introduced rank-weighted combat penalties, and eliminated double-counting in the turn-pair transition mechanism.

\paragraph{Positional and Anti-Stall Mechanisms.}
Beyond combat, the system implements specific shaping to guide behavior. A constant base step penalty of $-0.002$ creates time pressure toward efficiency. For stalemate prevention, sudden mobility restrictions (dropping below 5 legal moves) trigger a $-0.05$ penalty, discouraging the agent from boxing itself into immovable positions. Strategic proximity shaping grants a $+0.1$ bonus for moving pieces toward the opponent's back-rank corners, guiding the agent toward typical flag locations. A per-episode curiosity tracker provides decaying novelty bonuses for visiting new board states, and resets at episode boundaries to maintain consistent exploration pressure throughout training.
