...
The choice of a normalized range $[V_{min}, V_{max}] = [-3, 3]$ is fundamentally linked to the use of C51 distributional RL. A narrow, well-defined support allows the 51 atoms to resolve fine-grained differences in value estimates ($\delta_z \approx 0.12$). If rewards were not normalized, large cumulative returns would exceed the support bounds, leading to "saturation" where the agent cannot distinguish between varying high-value outcomes. Specifically:
    \item \textbf{Terminal Outcomes}: Terminal rewards are normalized to $\pm 1.0$ for wins and losses. A draw receives $-0.5$, establishing it as an outcome that is strategically inferior to a victory but superior to an outright loss, thereby providing a more nuanced gradient for games that reach the maximum step count.
    \item \textbf{High-Level Time Pressure}: To incentivize efficient victories, a non-linear step penalty is applied based on the episode duration. The penalty begins at $-0.0001$ per step, increasing to $-0.0002$ after 500 steps, and reaching $-0.0005$ after 1000 steps. This refined scaling ensures that the "urgency" signal remains present without accumulating to a magnitude that overshadows the primary reward for winning.
    \item \textbf{Positional Constraints}: To prevent "movement farming," positional rewards for territory advancement are state-based rather than move-based. The agent receives a one-time bonus of $+0.05$ only when it reaches a new row closer to the enemy base for the first time in an episode.
    \item \textbf{Sparse Reward Mitigation}: In Stratego, a game can last hundreds of moves without a capture. Dense rewards for information gain and territory advancement (upgraded to $+0.02$ per forward step) provide a continuous gradient that guides the agent toward the enemy base even when the ultimate win reward is distant.
    \item \textbf{Risk Sensitivity}: By including epistemic rewards (bonuses for revealing pieces), the distributional network learns the variance inherent in attacking unknown pieces, allowing for more risk-aware strategic decisions.
\end{itemize}
