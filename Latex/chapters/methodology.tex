...
This model-free approach offers several advantages over explicit belief tracking. The agent jointly learns both what embeddings to produce from action sequences and how to interpret these embeddings for decision-making. This eliminates the need for hand-crafted Bayesian update rules and constraint enforcement, instead allowing the network to discover effective representations through end-to-end training.
...
The Q-network employs a Deep Residual Network (ResNet) backbone to capture long-range spatial dependencies across the board. The architecture consists of an initial convolutional layer (64 filters, $3 \times 3$ kernel) followed by six residual blocks (twelve convolutional layers total), each containing two convolutional layers with Instance Normalization and ReLU activation. A spatial self-attention layer follows the ResNet backbone to enable global board reasoning, allowing each position to attend to all other positions for capturing long-range piece relationships. This deep feature extractor feeds into the dueling streams, allowing for more abstract strategic reasoning compared to shallow architectures. The 79-channel state representation is partitioned into two components, as detailed in Table \ref{tab:state_channels}.
