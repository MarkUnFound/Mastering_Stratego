...
The training configuration uses several key parameters: learning rate $\alpha = 10^{-4}$ with AdamW optimizer and weight decay $\lambda = 0.01$, and a discount factor $\gamma = 0.99$ to capture long-term strategic consequences. We utilize a batch size of 128 transitions and a replay buffer capacity of 500,000 transitions with prioritized sampling. Soft target network updates ($\tau = 0.001$) occur every 5,000 steps. Model updates are performed every step utilizing 8 parallel lanes and 5-step returns to improve credit assignment. To improve sample efficiency, data augmentation is applied via horizontal flip, exploiting the game's symmetry. Training proceeds for 35,000 episodes with checkpoints saved every 500 episodes.
