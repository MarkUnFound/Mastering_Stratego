...
The training configuration uses the following key parameters: learning rate $\alpha = 10^{-5}$ with AdamW optimizer and weight decay $\lambda = 0.01$, discount factor $\gamma = 0.95$, batch size of 256 transitions, replay buffer capacity of 80,000 transitions with prioritized sampling, and soft target network updates ($\tau = 0.001$) every 5,000 steps. The model updates are performed every 16 steps using 3-step returns to balance training throughput with sample efficiency. Training proceeds for 35,000 episodes with model checkpoints saved every 250 episodes.
...
Training follows a five-phase curriculum of increasing complexity with extended episode counts to ensure thorough learning at each stage. Phase 1 (Full Observability) allows the agent to observe all piece identities, enabling acquisition of fundamental game mechanics. Training proceeds for 5,000--10,000 episodes against a progressively challenging opponent mix: initially random opponents, then basic heuristic agents, and finally a sophisticated multi-factor heuristic opponent that evaluates combat outcomes, piece protection, positional control, and tactical considerations such as Scout reconnaissance and Miner prioritization. Phase advancement requires $\geq$95\% win rate against random opponents, $\geq$75\% against the basic heuristic, and $\geq$60\% against the strategic heuristic.
...
\paragraph{Distributional RL-Compatible Reward Normalization.}
The C51 distributional architecture requires careful reward scaling to ensure cumulative returns fall within the value distribution support $[V_{min}, V_{max}] = [-12, +12]$. With 51 atoms, each atom spans approximately 0.48 value units ($\delta_z = 24/50$), providing sufficient resolution for learning the return distribution while accommodating terminal rewards of $\pm 10$.

To address reward hacking behavior---where the agent learns to stall games and force draws due to risk aversion toward unknown enemy piece ranks---the framework implements an anti-stall reward structure with normalized magnitudes. Terminal rewards are set to $+1.0$ for wins and $-1.0$ for losses, well within the distribution bounds. Draws and timeouts receive $-0.8$, deliberately \textit{worse} than a fighting loss, which creates a gradient toward aggressive play.
...
Beyond combat and information rewards, two positional shaping components guide territorial behavior. The flag proximity bonus provides $+0.03$ for moves that position pieces within the opponent's back two rows, with higher weights for corner positions where flags are typically placed. This creates a gradient toward the objective without requiring explicit flag detection. Additionally, a mobility reward of $+0.001 \cdot n_{moves}$ encourages maintaining positions with high action availability, as pieces with more valid moves occupy strategically flexible positions.
...
\noindent where $a^* = \arg\max_a Q(s_{t+n}, a)$ under Double Q-learning. With $n=3$ and $\gamma = 0.95$, the effective discount for the bootstrapped value is $\gamma^3 = 0.857$. This configuration reduces the time constant for reward propagation while maintaining bias-variance balance. The three-step return collects actual rewards over a short trajectory before bootstrapping, which is particularly beneficial in Stratego where the consequences of tactical decisions often manifest over several moves.
...
\noindent where $\alpha_0 = 10^{-5}$ is the initial learning rate and $e$ is the episode count. This schedule enables aggressive early learning while maintaining stability as the policy approaches convergence. The decay rate was selected to provide approximately 35\% learning rate reduction over 10,000 episodes.
