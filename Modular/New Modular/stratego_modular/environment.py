...
class StrategoEnvironment:
    ...
    def reset(self, p1_placement: Optional[List[Tuple[PieceType, Tuple[int, int]]]] = None,
        ...
        if hasattr(self, '_cached_piece_counts'):
            self._cached_piece_counts = {1: 40, -1: 40}  # Initial piece count
    ...
    def _get_p1_positions(self) -> List[Tuple[int, int]]:
        ...
        # OPTIMIZATION: Pre-compute lake positions once (avoid repeated .item() calls)
    ...
    def _get_p2_positions(self) -> List[Tuple[int, int]]:
        ...
        # OPTIMIZATION: Reuse pre-computed lake positions (avoid repeated .item() calls)
        if not hasattr(self, '_lake_positions_set'):
            self._lake_positions_set = set((int(r.item()), int(c.item())) for r, c in self.board.lakes)
        # Remove lake positions (shouldn't be any, but check anyway)
        positions = [pos for pos in positions if pos not in self._lake_positions_set]
    ...
    def get_valid_moves(self) -> List[Tuple[Tuple[int, int], Tuple[int, int]]]:
        ...
        # OPTIMIZATION: Convert directions to Python tuples once (avoid repeated .item() calls)
        directions_list = [(int(dr.item()), int(dc.item())) for dr, dc in self.directions]
        
        # OPTIMIZATION: Convert player_pieces to Python list once (avoid repeated .item() calls)
        player_pieces_list = [(int(r.item()), int(c.item())) for r, c in player_pieces]
        
        for r, c in player_pieces_list:
            # OPTIMIZATION: Use tensor indexing directly, convert to int only once
            ...
            if piece_type == PieceType.SCOUT:
                for dr, dc in directions_list:
                    for i in range(1, BOARD_SIZE):
                        ...
                        # OPTIMIZATION: Use tensor indexing, convert to int only once
    ...
    def step(self, action: Tuple[Tuple[int, int], Tuple[int, int]]) -> Tuple[GameState, float, bool, Dict]:
        ...
        if target_piece_value != EMPTY_SQUARE and target_piece_value != LAKE_SQUARE:
            ...
            if result == 1:  # Attacker wins
                ...
                # OPTIMIZATION: Invalidate piece count cache when piece is captured
                if hasattr(self, '_cached_piece_counts'):
                ...
                # OPTIMIZATION: Invalidate piece count cache when piece is lost
                if hasattr(self, '_cached_piece_counts'):
                    self._cached_piece_counts[self.current_player] = max(0, self._cached_piece_counts[self.current_player] - 1)
                ...
                # OPTIMIZATION: Invalidate piece count cache when both pieces are removed
                if hasattr(self, '_cached_piece_counts'):
                    self._cached_piece_counts[self.current_player] = max(0, self._cached_piece_counts[self.current_player] - 1)
                    self._cached_piece_counts[-self.current_player] = max(0, self._cached_piece_counts[-self.current_player] - 1)
            ...
            # 5. NEW: Endgame Behavior Rewards - Reward aggressive moves toward enemy flag
            if game_phase == "end":
        ...
        # OPTIMIZATION: Cache flag positions to avoid scanning entire board every move
        ...
        if not self.game_over:
            # OPTIMIZATION: Cache flag position instead of scanning entire board
            flag_pos = None
            ...
            if self._flag_positions[self.current_player] is None or self.turn_count % 10 == 0:
                # OPTIMIZATION: Use tensor operations instead of Python loop with .item() calls
                # This is much faster for finding flag position
                if self.current_player == 1:
            ...
            # 4. NEW: Enhanced Flag Protection Throughout Game
            if flag_pos:
            ...
            # OPTIMIZATION: Use vectorized operations where possible, minimize .item() calls
            for dr in [-1, 0, 1]:
                for dc in [-1, 0, 1]:
                    if dr == 0 and dc == 0:
                        continue
            ...
            # 3. NEW: Threat Assessment / Vulnerability Penalties
        ...
        # OPTIMIZATION: Cache piece counts to avoid recomputing every move
        # NEW: Piece advantage reward (reward for having more pieces than opponent)
        if not self.game_over:
            # OPTIMIZATION: Only recompute piece counts every 5 moves (they don't change every move)
            ...
            # 1. NEW: Piece Value Advantage (Not Just Count)
    ...
    def _check_game_end(self):
        ...
        # OPTIMIZATION: Use cached flag positions instead of scanning entire board
        ...
        # OPTIMIZATION: Only check valid moves if flags exist (avoid expensive call if game already over)
        if not self.game_over:
    ...
    def _get_game_state(self) -> GameState:
        ...
        # OPTIMIZATION: Only copy move_history and revealed_pieces if they're actually used
        # Most of the time, these are just passed through without modification
        ...
        # Note: clone() is necessary here to avoid reference issues when board changes
