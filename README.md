...
The decision engine upgrades Vanilla DQN to a full **21.7M parameter Rainbow DQN**:
...
At test time, MARQ resolves this through a **Formal Expectamax Search Engine** (`dqn_bot_logic.py`) operating over a 4-ply depth bounded by a 4.0-second safety timeout:
...
2. **Chance Nodes (Attacking Hidden Pieces)**: When a move initiates combat against a face-down enemy piece, standard minimax cannot resolve the outcome. Expectamax queries the **AAREN posterior rank distribution** $P(\text{rank}_j \mid \mathcal{H})$:
...
│   │   ├── dqn_models/                 # Rainbow DQN, Dueling heads, Noisy layers
...
│   ├── stratego_game_improved.py       # Main graphical interface (Human vs AI, AI vs AI)
│   ├── dqn_bot_logic.py                # Expectamax Search & DQN bridge
│   ├── bot_logic.py                    # Bot interface adapter for Pygame
│   ├── verify_expectamax.py            # Unit tests for Expectamax combat utilities
│   ├── agent1_league_episode_1000.pt   # Pre-trained Rainbow DQN + AAREN checkpoint
...
│
├── aaren_architecture.drawio.xml       # Structural vector diagram for AAREN
├── marq_framework_arch.drawio.xml      # Full end-to-end framework diagram
├── marq_training_pipeline.drawio.xml   # Training and data flow diagram
...
### 2. Play Against the Trained Agent (GUI)
Launch the interactive Pygame board featuring the trained Rainbow DQN agent integrated with Expectamax test-time search:

```bash
cd "Python Stratego Game"
python stratego_game_improved.py
```

### 3. Verify Expectamax Logic
Run the formal verification test verifying that Chance Nodes accurately evaluate combat outcomes:
...
Hyperparameters can be adjusted in `Modular Stratego/settings/training_config.py`.

### 5. Benchmark & Mining Suite
To parse historical JSON logs and reproduce comparative win-rate curves:

```bash
cd "Modular Stratego"
python analyze_results_for_paper.py
```
